...
/// @title Sapient Strategy V40 — V34 + silence risk
/// @notice Same as V34 (YQ baseline) except: first trade after long silence gets a one-off fee scale (stale quote → higher arb risk).
/// @dev Unique angle 2.4 from docs/2025-02-10-V34-explanation-and-unique-angles.md. No new slots; uses elapsed at step boundary.
contract Strategy is AMMStrategyBase {
    ...
    // --- Silence risk (angle 2.4) ---
    uint256 constant SILENCE_COEF = 2e15; // 0.2% scale per elapsed step (WAD)
    uint256 constant SILENCE_ELAPSED_CAP = 8;
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        // Silence risk: first trade after long silence — scale both sides up (stale quote → arb risk)
        if (firstInStep && silenceElapsed > 0) {
            uint256 capped = silenceElapsed > SILENCE_ELAPSED_CAP ? SILENCE_ELAPSED_CAP : silenceElapsed;
            uint256 bump = wmul(SILENCE_COEF, capped);
            bidFee = clampFee(wmul(bidFee, WAD + bump));
            askFee = clampFee(wmul(askFee, WAD + bump));
        }
    ...
    function getName() external pure override returns (string memory) {
        return "Sapient v40 - silence risk";
    }
}

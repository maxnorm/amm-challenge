...
/// @title Sapient Strategy V15 — V14 + tail compression
/// @notice Same as V14 (low base + additive) but replaces hard cap with tail compression: above TAIL_KNEE (5 bps), fee = knee + slope×(fee − knee); protect vs attract by reserve imbalance; then clamp to 85 bps.
/// @dev See /docs/2025-02-09-Sapient-V14-amm-fee-designer-analysis.md
contract Strategy is AMMStrategyBase {
    ...
    uint256 constant MAX_FEE_CAP = 85e14;        // 85 bps cap (after compression)
    ...
    // V15 — Tail compression (knee + slope, then clamp; like YQ)
    uint256 constant TAIL_KNEE = 5e14;            // 5 bps — compress above this
    uint256 constant TAIL_SLOPE_PROTECT = 93e16;  // 0.93 for protect side
    uint256 constant TAIL_SLOPE_ATTRACT = 955e15; // 0.955 for attract side
    ...
    /// @dev Tail compression: above knee, fee = knee + slope×(fee - knee)
    ...
    /// @notice V15: V14 + tail compression (knee 5 bps, slopes 0.93/0.955); protect/attract by reserve imbalance; clamp 85 bps
    function afterSwap(TradeInfo calldata trade) external override returns (uint256 bidFee, uint256 askFee) {
        ...
        // V15 — Tail compression: protect side = side we charge more (vulnerable); use reserve imbalance as proxy (no dirState in V14)
    ...
    function getName() external pure override returns (string memory) {
        return "Sapient v15 - (V14 + tail compression)";
    }
}

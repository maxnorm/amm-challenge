...
/// @title Sapient Strategy V24 — V23 + activity in the base
/// @notice Same as V23 (3 bps base, tail compression) but add YQ-style activity in base: lambdaHat, flowSize (lambda×size), actEma. Step-based decay; blend on trade when tradeRatio > threshold.
/// @dev Tests whether "low base + activity build-up" (like YQ) improves edge vs V23 (~380) without the full V22 stack that regressed to 128.
contract Strategy is AMMStrategyBase {
    ...
    uint256 constant LAMBDA_COEF = 12e14;     // 12 bps
    uint256 constant FLOW_SIZE_COEF = 4842e14;
    uint256 constant ACT_COEF = 91843e14;
    ...
    function getName() external pure override returns (string memory) {
        return "Sapient v24 - (V23 + activity in base)";
    }
}

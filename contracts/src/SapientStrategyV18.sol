...
/// @title Sapient Strategy V18 — V14 + pImplied only
/// @notice Same as V14 (low base + additive, 75 bps cap) but uses fee-adjusted price (pImplied) for ret and pHat update; prev bid/ask stored. No dirState, stale, trade boost, or sigma×tox/cubic.
/// @dev See /docs/2025-02-09-Sapient-V18-pImplied-only-plan.md
contract Strategy is AMMStrategyBase {
    ...
    /// @notice V18: V14 + pImplied only — fee-adjusted price for ret and pHat; prev bid/ask stored; no other levers
    ...
    function getName() external pure override returns (string memory) {
        return "Sapient v18 - (V14 + pImplied only)";
    }
}

...
/// @title Sapient Strategy V28 — V27 + pHat write order fix (match V23)
/// @notice V27 still 40.30: _computeSpotPhatRetGate wrote new pHat to slot before vol computation, so vol used new pHat. V23 uses old pHat for vol then writes pHat at end. V28: don't write pHat in helper; return it and write after vol so vol uses old pHat.
/// @dev Root cause of V24–V27 regression: vol = alpha*|spot-pHat|/pHat + ...; with new pHat (moved toward spot) vol is smaller → different fee path.
contract Strategy is AMMStrategyBase {
    ...
    function getName() external pure override returns (string memory) {
        return "Sapient v28 - (V27 pHat write order fix)";
    }
}

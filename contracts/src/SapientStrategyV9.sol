...
/// @title Sapient Strategy V9 — V8 tuned: cap 75, no stale/attract, no dirState skew
/// @notice Same as V8 but MAX_FEE_CAP=75 bps; STALE_COEF=0, ATTRACT_FRAC=0; DIR_COEF=0, DIR_TOX_COEF=0. Isolates whether cap/stale/dirState skew caused the V8 regression.
/// @dev See /docs/2025-02-09-Sapient-V9-tune-changelog.md
contract Strategy is AMMStrategyBase {
    ...
    uint256 constant MAX_FEE_CAP = 75e14;        // V9 TUNE: back to 75 bps (was 85 in V8)
    ...
    // V9 TUNE: Stale + attract disabled (was 68e14, 1124e15)
    uint256 constant STALE_COEF = 0;
    uint256 constant ATTRACT_FRAC = 0;

    // V8 Phase 2 — dirState and time-consistent decay
    ...
    // V9 TUNE: dirState skew disabled (was 20e14, 10e14)
    uint256 constant DIR_COEF = 0;
    uint256 constant DIR_TOX_COEF = 0;
    ...
    /// @dev Applies trade-aligned toxicity boost and stale+attract spread (V9: stale/attract zeroed)
    function _applyTradeBoostAndStaleAttract(
        ...
        // Stale + attract: V9 STALE_COEF=0 so staleShift=0, no fee change here
    ...
    /// @dev Applies dirState skew (V9: DIR_COEF=0, DIR_TOX_COEF=0 so skew=0, no fee change)
    ...
    /// @notice V9: V8 tuned — cap 75, no stale/attract, no dirState skew; rest unchanged
    ...
    function getName() external pure override returns (string memory) {
        return "Sapient v9 - (V8 tuned: cap 75, no stale-attract, no dirState skew)";
    }
}

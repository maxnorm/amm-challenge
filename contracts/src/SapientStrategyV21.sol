...
/// @title Sapient Strategy V21 — Reverted to V20 baseline (first-in-step regressed to 128)
/// @notice Same as V20: V14 + trade-aligned toxicity boost. First-in-step (pHat dual alpha) was tried and regressed edge to 128; sigma-every-trade fix did not recover. Use V20 logic until next structural try (e.g. tail compression).
/// @dev See /docs/2025-02-10-Sapient-V21-first-in-step-changelog.md
contract Strategy is AMMStrategyBase {
    ...
    /// @dev Applies trade boost reading reserveY/amountY/isBuy from temp slots (reduces stack)
    ...
    /// @notice V21: reverted to V20 logic (first-in-step regressed to 128)
    ...
    function getName() external pure override returns (string memory) {
        return "Sapient v21 - (V20 baseline, first-in-step reverted)";
    }
}

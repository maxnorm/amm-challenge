...
/// @title Sapient Strategy V13 — V11 + first-in-step pHat/sigma
/// @notice Same as V11 but: use fast alpha (PHAT_ALPHA) on first trade in a step, slow alpha (PHAT_ALPHA_RETAIL) otherwise; update sigmaHat only on first-in-step (as in audit / YQ).
/// @dev See /docs/2025-02-09-Sapient-V13-first-in-step-changelog.md
contract Strategy is AMMStrategyBase {
    ...
    uint256 constant PHAT_ALPHA = 26e16;         // 0.26 fast (first-in-step)
    uint256 constant PHAT_ALPHA_RETAIL = 5e16;   // 0.05 slow (later trades in step)
    ...
    // V11 — Activity/flow in base (tuned down from V10 + cap)
    uint256 constant LAMBDA_DECAY = 99e16;       // 0.99
    uint256 constant LAMBDA_COEF = 6e14;         // 6 bps per unit lambdaHat (was 12)
    ...
    uint256 constant FLOW_SIZE_COEF = 15e14;     // 15 bps per unit flowSize (was 48)
    uint256 constant CAP_FLOW_BPS = 12e14;       // cap total flow term at 12 bps
    ...
    /// @notice V13: V11 + first-in-step pHat/sigma (fast alpha first trade, slow alpha otherwise; sigma only on first-in-step)
    function afterSwap(TradeInfo calldata trade) external override returns (uint256 bidFee, uint256 askFee) {
        ...
        uint256 pAlpha = firstInStep ? PHAT_ALPHA : PHAT_ALPHA_RETAIL;
        if (ret <= adaptiveGate) {
            pHat = _wmul(ONE_WAD - pAlpha, pHat) + _wmul(pAlpha, pImplied);
        }
        if (firstInStep) {
        ...
        uint256 flowBps = _wmul(LAMBDA_COEF, lambdaHat) + _wmul(FLOW_SIZE_COEF, flowSize);
        if (flowBps > CAP_FLOW_BPS) flowBps = CAP_FLOW_BPS;
        rawFee = rawFee + flowBps;
    ...
    function getName() external pure override returns (string memory) {
        return "Sapient v13 - (V11 + first-in-step pHat/sigma)";
    }
}

...
/// @title Sapient Strategy V33 — V28 but fee path never reads slots 5,6,7 (isolate bug)
/// @notice If reading slots 5,6,7 in _computeRawFeeAdditive (even when coefs=0) causes 40, then not reading them should restore 380. Activity block still runs and writes 5,6,7,8.
/// @dev Remove activity terms from raw fee so fee formula is byte-identical to V23 (no SLOAD of 5,6,7).
contract Strategy is AMMStrategyBase {
    ...
    function _computeRawFeeAdditive(uint256 vol, uint256 sigmaHat, uint256 toxEma, uint256 lastTs) private view returns (uint256 rawFee) {
        ...
        // V33: do NOT read slots 5,6,7 in fee path (no activity terms) to test if that fixes 40.30
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256 bidFee, uint256 askFee) {
        ...
        // Use old pHat (slot not yet updated) for vol, matching V23 order
    ...
    function getName() external pure override returns (string memory) {
        return "Sapient v33 - (V28 fee path no slot 5,6,7 read)";
    }
}

...
/// @title Sapient Strategy V19 — V14 + sigma×tox + cubic tox (vulnerable side)
/// @notice Same as V14 (low base + additive, 75 bps cap) but adds SIGMA_TOX_COEF * sigmaHat * toxEma and TOX_CUBIC_COEF * toxEma^3 to vulnerable-side tox. No new slots.
/// @dev See /docs/2025-02-09-Sapient-other-levers-research.md Section C; /docs/2025-02-09-Sapient-V18-pImplied-only-plan.md §6
contract Strategy is AMMStrategyBase {
    ...
    uint256 constant SIGMA_TOX_COEF = 50e14;     // 50 bps per sigma×tox (vulnerable side)
    uint256 constant TOX_CUBIC_COEF = 15e14;     // 15 bps per tox^3 (vulnerable side)
    ...
    /// @notice V19: V14 + sigma×tox + cubic tox on vulnerable side; same pipeline otherwise
    function afterSwap(TradeInfo calldata trade) external override returns (uint256 bidFee, uint256 askFee) {
        ...
        // Toxicity premium on vulnerable side only: linear + quad + sigma×tox + cubic
    ...
    function getName() external pure override returns (string memory) {
        return "Sapient v19 - (V14 + sigma-tox + cubic tox)";
    }
}

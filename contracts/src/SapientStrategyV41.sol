...
/// @title Sapient Strategy V41 — V34 + gate by trade size
/// @notice Same as V34 (YQ baseline) except: when trade size is large, adaptive gate is widened so we don't trust one big move for pHat.
/// @dev Unique angle 2.3 from docs/2025-02-10-V34-explanation-and-unique-angles.md. No new slots.
contract Strategy is AMMStrategyBase {
    ...
    // --- Gate by size (angle 2.3): wider gate when trade is large ---
    uint256 constant LARGE_TRADE_THRESH = 1e16; // 1% of reserve (WAD)
    uint256 constant GATE_LARGE_MULT = 15e17; // 1.5 WAD — gate 50% wider for large trades
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        // Compute tradeRatio early so we can use it in the gate (angle 2.3)
...

        {
            ...
            // Large trade: wider gate so we don't trust one big move for pHat
            if (tradeRatio >= LARGE_TRADE_THRESH) adaptiveGate = wmul(adaptiveGate, GATE_LARGE_MULT);
            if (ret <= adaptiveGate) {
    ...
    function getName() external pure override returns (string memory) {
        return "Sapient v41 - gate by size";
    }
}

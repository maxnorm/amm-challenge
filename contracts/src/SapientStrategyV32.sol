...
/// @title Sapient Strategy V32 — V28 + activity block after price/vol (fix order bug)
/// @notice In V24–V31 the activity block (step decay, blend, step count) ran BEFORE spot/pHat/vol. That may have affected fee path. V32: compute spot/pHat/sigma/vol/tox and set temp slots first (same as V23), THEN run activity block, THEN rawFee. Activity state is updated with current trade but only used for fee after price state is fixed.
/// @dev Bug fix: activity logic must not run before price/vol so it cannot inadvertently affect them.
contract Strategy is AMMStrategyBase {
    ...
    uint256 constant SLOT_TEMP_PHAT = 19;
    ...
    /// @dev Compute spot, update pHat/sigma, store spot/ret/adaptiveGate/newPHat in temp slots. Returns new sigmaHat. Does NOT write SLOT_PHAT so caller can use old pHat for vol then write new pHat (V23 order).
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256 bidFee, uint256 askFee) {
        ...
        // Price/vol/tox first (same order as V23) — no activity block before
        ...
        // Activity block after price/vol/temp slots, before fee (fix: must not affect spot/pHat/vol)
        ...
        (bidFeeOut, askFeeOut) = _applyDirSurgeAndSize(ret, slots[SLOT_TEMP_GATE], spot, slots[SLOT_TEMP_PHAT], bidFeeOut, askFeeOut);
        (bidFeeOut, askFeeOut) = _applyTradeBoostFromSlots(bidFeeOut, askFeeOut, spot, slots[SLOT_TEMP_PHAT]);
        (bidFeeOut, askFeeOut) = _applyTailCompression(bidFeeOut, askFeeOut);

        slots[SLOT_PHAT] = slots[SLOT_TEMP_PHAT];
    ...
    function getName() external pure override returns (string memory) {
        return "Sapient v32 - (V28 + activity after price/vol)";
    }
}

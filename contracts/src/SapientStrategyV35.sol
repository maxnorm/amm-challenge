...
/// @title Sapient Strategy V35 — V34 + imbalance skew
/// @notice Same as V34 (YQ baseline) plus a small inventory-based skew: when reserveY >= reserveX we protect bid (add to bid, subtract from ask); else protect ask. Coefficient small so we reinforce dirState with inventory without changing base.
/// @dev See docs/2025-02-10-Sapient-V35-imbalance-skew-changelog.md.
contract Strategy is AMMStrategyBase {
    ...
    // --- Imbalance skew (inventory-based; max ~15 bps at full imbalance) ---
    uint256 constant IMB_SKEW_COEF = 15 * BPS;
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        // Imbalance skew: inventory-based. reserveY >= reserveX => we're long Y => arb sells us X => protect bid.
        {
            uint256 totalReserves = trade.reserveX + trade.reserveY;
            uint256 imbalance = totalReserves > 0 ? wdiv(absDiff(trade.reserveX, trade.reserveY), totalReserves) : 0;
            uint256 imbSkew = wmul(IMB_SKEW_COEF, imbalance);
            if (trade.reserveY >= trade.reserveX) {
                bidFee = bidFee + imbSkew;
                askFee = askFee > imbSkew ? askFee - imbSkew : 0;
            } else {
                askFee = askFee + imbSkew;
                bidFee = bidFee > imbSkew ? bidFee - imbSkew : 0;
            }
        }
    ...
    function getName() external pure override returns (string memory) {
        return "Sapient v35 - YQ baseline + imbalance skew";
    }
}

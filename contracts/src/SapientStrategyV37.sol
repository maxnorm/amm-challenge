...
/// @title Sapient Strategy V37 — V36 with looser calm thresholds
/// @notice Same as V36 (calm vs stressed) but calm = sigma <= 2% and tox <= 5% (so we're calm more often). Discount and stressed bump unchanged.
/// @dev See docs/2025-02-10-Sapient-V37-calm-thresholds-changelog.md.
contract Strategy is AMMStrategyBase {
    ...
    // --- Calm vs stressed: LOOSER thresholds (calm more often) vs V36 ---
    uint256 constant CALM_SIGMA_THRESH = 2e16;   // 2% (V36 was 1%)
    uint256 constant CALM_TOX_THRESH = 5e16;    // 5% (V36 was 2%)
    uint256 constant CALM_ATTRACT_DISCOUNT = 97e16; // 0.97
    uint256 constant STRESSED_PROTECT_BPS = 3 * BPS;
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        // Calm vs stressed (looser thresholds): calm more often => more attract discount
        {
    ...
    function getName() external pure override returns (string memory) {
        return "Sapient v37 - calm vs stressed (looser thresh)";
    }
}

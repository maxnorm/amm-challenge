...
/// @title Sapient Strategy V12 — V8 + tail compression
/// @notice Same as V8 but replaces hard cap with tail compression: above TAIL_KNEE (5 bps), fee = knee + slope×(fee - knee); different slopes for protect vs attract side, then clamp to 85 bps.
/// @dev See /docs/2025-02-09-Sapient-V12-changelog.md, /docs/2025-02-09-YQ-extract-for-Sapient.md (Tail compression)
contract Strategy is AMMStrategyBase {
    ...
    // V12 — Tail compression (knee + slope, then clamp; like YQ)
    ...
    /// @notice V12: V8 + tail compression (knee 5 bps, slopes 0.93/0.955), then clamp 85 bps
    function afterSwap(TradeInfo calldata trade) external override returns (uint256 bidFee, uint256 askFee) {
        ...
        // V12 — Tail compression: above knee, compress with slope; protect vs attract by dirState
    ...
    function getName() external pure override returns (string memory) {
        return "Sapient v12 - (V8 + tail compression)";
    }
}

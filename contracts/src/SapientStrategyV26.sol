...
/// @title Sapient Strategy V26 — V25 + scaled LAMBDA_COEF
/// @notice V25 stayed at edge 40.30: LAMBDA_COEF×lambdaHat adds up to 60 bps when lambdaHat=5 (many trades/step). Scale LAMBDA_COEF to 2e14 so max 10 bps from lambda; total activity ~5–20 bps.
/// @dev See docs/2025-02-10-V24-edge-40-investigation.md and V25 result. All three activity coefs (lambda, flowSize, actEma) now scaled.
contract Strategy is AMMStrategyBase {
    ...
    uint256 constant LAMBDA_COEF = 2e14;      // 2 bps per unit; max 10 bps when lambdaHat=5 (was 12e14 → 60 bps)
    ...
    function getName() external pure override returns (string memory) {
        return "Sapient v26 - (V25 + scaled lambda)";
    }
}

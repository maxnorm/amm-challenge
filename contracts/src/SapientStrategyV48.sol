...
/// @title Sapient Strategy V48 — V34 + dual sigma + dual lambda (P3)
/// @notice Same as V34 plus: sigma_slow and lambda_slow (slower EMAs). Fee bump when sigma/sigma_slow > 1.1 (vol regime) or lambda/lambda_slow > 1.2 (activity regime).
/// @dev P3 from docs/2025-02-10-next-formulas-innovative-research.md. Slots 11 = sigma_slow, 12 = lambda_slow.
contract Strategy is AMMStrategyBase {
    ...
    // --- Dual sigma / lambda: slower EMAs for regime detection ---
    uint256 constant SIGMA_SLOW_DECAY = 950000000000000000;  // 0.95 (slower than SIGMA_DECAY 0.824)
    uint256 constant LAMBDA_SLOW_DECAY = 950000000000000000; // 0.95 (slower than LAMBDA_DECAY 0.99)
    uint256 constant VOL_REGIME_THRESH = 11e17;              // 1.1 WAD — sigma/sigma_slow > this -> vol regime
    uint256 constant ACTIVITY_REGIME_THRESH = 12e17;        // 1.2 WAD — lambda/lambda_slow > this -> activity regime
    uint256 constant VOL_REGIME_COEF = 2 * BPS;
    uint256 constant ACTIVITY_REGIME_COEF = 2 * BPS;
    ...
    // slots[0..10] = same as V34
    // slots[11] = sigma_slow
    // slots[12] = lambda_slow
    ...
    function afterInitialize(uint256 initialX, uint256 initialY) external override returns (uint256, uint256) {
        ...
        slots[9] = 0;
        slots[10] = 0;
        slots[11] = 950000000000000;   // sigma_slow = same as initial sigma
        slots[12] = 800000000000000000; // lambda_slow = same as initial lambda
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        uint256 sigmaSlow = slots[11];
        uint256 lambdaSlow = slots[12];
        ...
        if (isNewStep) {
            ...
            if (stepTradeCount > 0 && elapsedRaw > 0) {
                ...
                lambdaSlow = wmul(lambdaSlow, LAMBDA_SLOW_DECAY) + wmul(lambdaHat, WAD - LAMBDA_SLOW_DECAY);
            }
...

        {
            ...
            if (firstInStep) {
                ...
                sigmaSlow = wmul(sigmaSlow, SIGMA_SLOW_DECAY) + wmul(sigmaHat, WAD - SIGMA_SLOW_DECAY);
            }
        }
        ...
        // Regime bumps: vol (sigma > 1.1 * sigma_slow), activity (lambda > 1.2 * lambda_slow)
        if (sigmaSlow > 0 && sigmaHat > wmul(sigmaSlow, VOL_REGIME_THRESH)) {
            fMid = fMid + VOL_REGIME_COEF;
        }
        if (lambdaSlow > 0 && lambdaHat > wmul(lambdaSlow, ACTIVITY_REGIME_THRESH)) {
            fMid = fMid + ACTIVITY_REGIME_COEF;
        }

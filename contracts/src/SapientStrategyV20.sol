...
/// @title Sapient Strategy V20 — V14 + trade-aligned toxicity boost
/// @notice Same as V14 (low base + additive, 75 bps cap) but when the current trade is toxic (buy when spot ≥ pHat or sell when spot < pHat), add a size-scaled boost to that side, capped at 25 bps. No new slots.
/// @dev See /docs/2025-02-09-Sapient-other-levers-research.md Section D; /docs/2025-02-09-Sapient-V18-pImplied-only-plan.md §6
contract Strategy is AMMStrategyBase {
    ...
    uint256 constant MAX_FEE_CAP = 75e14;       // guardrail: higher fees kill volume / select for toxic flow
    ...
    // V20 — Trade-aligned toxicity boost
    uint256 constant TRADE_TOX_BOOST = 25e14;   // 25 bps per unit trade ratio when toxic
    uint256 constant CAP_TRADE_BOOST = 25e14;   // 25 bps max boost
    ...
    /// @dev Applies trade-aligned toxicity boost: when current trade is toxic (buy when spot ≥ pHat or sell when spot < pHat), add capped boost to that side
    ...
    /// @notice V20: V14 + trade-aligned toxicity boost when current trade is toxic
    function afterSwap(TradeInfo calldata trade) external override returns (uint256 bidFee, uint256 askFee) {
        ...
        if (tradeRatio > TRADE_RATIO_CAP) tradeRatio = TRADE_RATIO_CAP;
        (bidFeeOut, askFeeOut) = _applyTradeAlignedBoost(bidFeeOut, askFeeOut, spot, pHat, tradeRatio, trade.isBuy);

...
/// @title Sapient Strategy V36 — V34 + calm vs stressed regime
/// @notice Same as V34 (YQ baseline) plus regime: calm (low sigma, low tox) => discount attract side; stressed => add to protect side. No new slots.
/// @dev See docs/2025-02-10-Sapient-V36-calm-stressed-changelog.md. Unique angle 2.2.
contract Strategy is AMMStrategyBase {
    ...
    // --- Calm vs stressed regime (2.2) ---
    uint256 constant CALM_SIGMA_THRESH = 1e16;   // 1% (WAD): sigma below => calm for sigma
    uint256 constant CALM_TOX_THRESH = 2e16;      // 2% (WAD): tox below => calm for tox
    uint256 constant CALM_ATTRACT_DISCOUNT = 97e16; // 0.97: attract fee *= this in calm
    uint256 constant STRESSED_PROTECT_BPS = 3 * BPS; // 3 bps added to protect side in stressed
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        // Calm vs stressed regime (2.2): calm => discount attract; stressed => add to protect
        {
            bool calm = (sigmaHat <= CALM_SIGMA_THRESH && toxSignal <= CALM_TOX_THRESH);
            if (calm) {
                // Lower fee on attract side to invite rebalancing
                if (sellPressure) {
                    askFee = wmul(askFee, CALM_ATTRACT_DISCOUNT);
                } else {
                    bidFee = wmul(bidFee, CALM_ATTRACT_DISCOUNT);
                }
            } else {
                // Stressed: wider spread by adding to protect side
                if (sellPressure) {
                    bidFee = bidFee + STRESSED_PROTECT_BPS;
                } else {
                    askFee = askFee + STRESSED_PROTECT_BPS;
                }
            }
        }
    ...
    function getName() external pure override returns (string memory) {
        return "Sapient v36 - YQ baseline + calm vs stressed";
    }
}

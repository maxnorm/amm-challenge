...
/// @title Sapient Strategy V8 — pImplied + sigma×tox + cubic + trade-boost + stale/attract + dirState + cap 85 bps
/// @notice V7 pipeline plus: pImplied for pHat/ret; sigma×tox and cubic tox; trade-aligned boost; stale+attract; dirState (flow-direction memory) and time-consistent tox decay; cap 85 bps.
/// @dev See /docs/2025-02-09-Sapient-V8-changelog.md, /docs/2025-02-09-Sapient-V8-design.md
contract Strategy is AMMStrategyBase {
    ...
    uint256 constant MAX_FEE_CAP = 85e14;        // V8: 85 bps (was 75) to reduce cap clipping
    ...
    // V8 — Sigma×tox and cubic tox (vulnerable side)
    ...
    // V8 — Trade-aligned toxicity boost
    uint256 constant TRADE_TOX_BOOST = 25e14;    // 25 bps per unit trade ratio
    uint256 constant CAP_TRADE_BOOST = 25e14;    // 25 bps max

    // V8 — Stale + attract (widen spread, attract rebalancing)
    uint256 constant STALE_COEF = 68e14;         // 68 bps per unit toxEma
    uint256 constant ATTRACT_FRAC = 1124e15;     // 1.124 in WAD — fraction of staleShift to subtract from other side

    // V8 Phase 2 — dirState (flow-direction memory) and time-consistent decay
    ...
    function _applyTradeBoostAndStaleAttract(
        ...
        // Trade-aligned boost: if current trade is toxic (buy when spot>=pHat or sell when spot<pHat), add boost to side hit
        ...
        // Stale + attract: add staleShift to vulnerable side (by move direction), subtract attract fraction from other
        uint256 staleShift = _wmul(STALE_COEF, toxEma);
        uint256 attractShift = _wmul(staleShift, ATTRACT_FRAC);
        if (spot >= pHat) {
            // Price moved up: bid (buy side) is the one we want to protect
    ...
    /// @notice V8: pImplied; base+sym-tox+floor+decay; tox+sigma×tox+cubic+asym; dir/surge/size; trade-boost; stale+attract; dirState skew; cap 85 bps
    function afterSwap(TradeInfo calldata trade) external override returns (uint256 bidFee, uint256 askFee) {
        ...
        // pImplied: fee-adjusted price for gate/pHat/sigma so one toxic trade doesn't drag pHat
        ...
        // Volatility from spot vs pHat (current move magnitude)
        slots[SLOT_TEMP_VOL] = _wmul(ALPHA, pHat > 0 ? _wdiv(_abs(spot, pHat), pHat) : 0) + _wmul(ONE_WAD - ALPHA, oldVol);

        // Toxicity: current step move (spot vs pHat) for toxEma
        ...
        // Toxicity premium on vulnerable side: linear + quad + sigma×tox + cubic
    ...
    function getName() external pure override returns (string memory) {
        return "Sapient v8 - (pImplied + sigma-tox + cubic + trade-boost + stale-attract + dirState + cap 85)";
    }
}

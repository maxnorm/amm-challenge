...
/// @title Sapient Strategy V39 — V34 + tox-dependent dirState decay
/// @notice Same as V34 (YQ baseline) except: when toxEma is high at step boundary, dirState decays faster toward neutral (stressed regime = don't carry stale direction).
/// @dev Unique angle 2.6 from docs/2025-02-10-V34-explanation-and-unique-angles.md. No new slots.
contract Strategy is AMMStrategyBase {
    ...
    uint256 constant DIR_DECAY_STRESSED = 600000000000000000; // 0.60 — faster decay when tox high
    uint256 constant TOX_DECAY_STRESSED_THRESH = 2e16; // 2% — above this use DIR_DECAY_STRESSED
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        if (isNewStep) {
            ...
            // Tox-dependent decay: when tox is high, decay dirState faster toward neutral
            uint256 dirDecay = toxEma >= TOX_DECAY_STRESSED_THRESH ? DIR_DECAY_STRESSED : DIR_DECAY;
            dirState = _decayCentered(dirState, dirDecay, elapsed);
    ...
    function getName() external pure override returns (string memory) {
        return "Sapient v39 - tox-dependent dir decay";
    }
}

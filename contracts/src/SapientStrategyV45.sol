...
/// @title Sapient Strategy V45 — V34 + size-weighted toxicity (CrocSwap-style)
/// @notice Same as V34 baseline plus: toxicity terms scaled by (1 + SIZE_TOX_COEF×sizeHat). Large trades get higher tox-based fees.
/// @dev See docs/2025-02-10-alternative-fee-formulas-research.md §6. Formula: toxWeighted = toxSignal × (WAD + SIZE_TOX_COEF×min(sizeHat,WAD)).
contract Strategy is AMMStrategyBase {
    ...
    // --- Size-weighted toxicity: tox term × (1 + SIZE_TOX_COEF×sizeHat) ---
    uint256 constant SIZE_TOX_COEF = 500000000000000000; // 0.5 WAD: at sizeHat=WAD, tox impact 1.5×
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        // Size-weighted toxicity: toxWeighted = toxSignal × (1 + SIZE_TOX_COEF×min(sizeHat,WAD))
        uint256 sizeCap = sizeHat > WAD ? WAD : sizeHat;
        uint256 sizeToxMult = WAD + wmul(SIZE_TOX_COEF, sizeCap);
        uint256 toxWeighted = wmul(toxSignal, sizeToxMult);
        ...
        fBase = fBase + wmul(ACT_COEF, actEma);

        // Toxicity terms use toxWeighted (size-weighted)
        uint256 fMid = fBase + wmul(TOX_COEF, toxWeighted) + wmul(TOX_QUAD_COEF, wmul(toxWeighted, toxWeighted));
        fMid = fMid + wmul(SIGMA_TOX_COEF, wmul(sigmaHat, toxWeighted));

        // Cubic toxicity (size-weighted)
        {
            uint256 toxCubed = wmul(toxWeighted, wmul(toxWeighted, toxWeighted));
        ...
        uint256 skew = wmul(DIR_COEF, dirDev) + wmul(DIR_TOX_COEF, wmul(dirDev, toxWeighted));
        ...
        // Directional protection using stale-price sign (toxWeighted)
        {
            uint256 staleShift = wmul(STALE_DIR_COEF, toxWeighted);

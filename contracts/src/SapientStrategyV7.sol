...
/// @title Sapient Strategy V7 — Approach A: dir-only-when-no-surge + scaled surge + sym-tox high + size bump
/// @notice Dir only when ret <= gate; scaled surge by (ret - gate); stronger symmetric tox above 1.5%; trade-size bump on side hit.
/// @dev See /docs/2025-02-09-V7-structural-brainstorm.md, /docs/Sapient-v6-edge-wall-deep-review.md
contract Strategy is AMMStrategyBase {
    ...
    /// @dev Computes raw fee (before cap); reads reserveX, reserveY, timestamp from temp slots to reduce caller stack
    ...
    /// @notice V7: base + sym-tox (+ high-tox term) + floor + decay; tox+asym; dir only when ret<=gate; scaled surge; size bump
    ...
    function getName() external pure override returns (string memory) {
        return "Sapient v7 - (dir-only-no-surge + scaled surge + sym-tox high + size bump)";
    }
}

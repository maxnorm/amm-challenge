...
/// @title Sapient Strategy V47 — V34 + momentum asymmetric fee (P2)
/// @notice Same as V34 plus: if last move was up (pImplied >= pHat), add MOMENTUM_COEF to ask; else add to bid. "Chase" the last move asymmetrically.
/// @dev P2 from docs/2025-02-10-next-formulas-innovative-research.md. One slot: slots[11] = lastRetSign (WAD = up, 0 = down).
contract Strategy is AMMStrategyBase {
    ...
    // --- Momentum asymmetric fee: bump ask when last move up, bid when down ---
    uint256 constant MOMENTUM_COEF = 2 * BPS; // 2 bps
    ...
    // slots[11] = lastRetSign (WAD = price moved up, 0 = down or unchanged)
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        uint256 retSignForNext = 0;
        {
            uint256 ret = pHat > 0 ? wdiv(absDiff(pImplied, pHat), pHat) : 0;
            if (pImplied >= pHat) retSignForNext = WAD; // price moved up
        ...
        // Momentum asymmetric fee: if last move was up, we're behind on ask -> bump ask; else bump bid
        if (lastRetSign >= WAD / 2) {
            askFee = clampFee(askFee + MOMENTUM_COEF);
        } else {
            bidFee = clampFee(bidFee + MOMENTUM_COEF);
        }
